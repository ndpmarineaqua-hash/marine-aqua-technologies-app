...
class SupportPage extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) => AppPage(title: 'Technical Support', subtitle: 'Connect with the Marine Aqua team',
    child: Column(children: const [
      SupportTile(Icons.call_outlined, 'Customer Care', '+91 93902 59830'),
      SizedBox(height: 10),
      SupportTile(Icons.email_outlined, 'Email', 'marineaquahr@gmail.com'),
      SizedBox(height: 10),
      SupportTile(Icons.location_on_outlined, 'Corporate Office', 'Madhapur, Hyderabad'),
    ]));
}

class SupportTile extends StatelessWidget {
  final IconData icon; final String title, value;
  const SupportTile(this.icon, this.title, this.value, {super.key});
  @override
  Widget build(BuildContext context) => Container(padding: const EdgeInsets.all(17),
    decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(18), border: Border.all(color: const Color(0xFFE0EFF1))),
    child: Row(children: [
      CircleAvatar(backgroundColor: const Color(0xFFE7F7F8), child: Icon(icon, color: const Color(0xFF007C91))),
      const SizedBox(width: 13),
      Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text(title, style: const TextStyle(fontWeight: FontWeight.w800)),
        const SizedBox(height: 3),
        Text(value, style: const TextStyle(color: Color(0xFF78909C), fontSize: 12)),
      ]),
    ]));
}
...
class NotificationsPage extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) => AppPage(title: 'Notifications', subtitle: 'Latest updates',
    child: const Card(child: Padding(padding: EdgeInsets.all(25), child: Center(child: Text('No new notifications')))));
}

class ProfilePage extends StatelessWidget {
  const ProfilePage({super.key});
  @override
  Widget build(BuildContext context) => AppPage(title: 'Profile', subtitle: 'Your Marine Aqua account',
    child: Column(children: [
      Container(width: double.infinity, padding: const EdgeInsets.all(22),
        decoration: BoxDecoration(gradient: const LinearGradient(colors: [Color(0xFF005B70), Color(0xFF009D9A)]), borderRadius: BorderRadius.circular(22)),
        child: const Row(children: [
          CircleAvatar(radius: 30, backgroundColor: Colors.white24, child: Icon(Icons.person, color: Colors.white, size: 32)),
          SizedBox(width: 15),
          Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text('Farmer Account', style: TextStyle(color: Colors.white, fontSize: 18, fontWeight: FontWeight.w800)),
            SizedBox(height: 4),
            Text('MARINE AQUA TECHNOLOGIES', style: TextStyle(color: Colors.white70, fontSize: 11)),
          ]),
        ])),
      const SizedBox(height: 15),
      const ProfileItem(Icons.notifications_none, 'Notifications'),
      const ProfileItem(Icons.help_outline, 'Help & Support'),
      const ProfileItem(Icons.info_outline, 'About MARINE AQUA TECHNOLOGIES'),
    ]));
}

class ProfileItem extends StatelessWidget {
  final IconData icon; final String title;
  const ProfileItem(this.icon, this.title, {super.key});
  @override
  Widget build(BuildContext context) => Container(margin: const EdgeInsets.only(bottom: 10), padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 15),
    decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(16), border: Border.all(color: const Color(0xFFE0EFF1))),
    child: Row(children: [Icon(icon, color: const Color(0xFF007C91)), const SizedBox(width: 14), Expanded(child: Text(title, style: const TextStyle(fontWeight: FontWeight.w600))), const Icon(Icons.chevron_right, color: Color(0xFF90A4AE))]));
}
...
class AppPage extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) => SafeArea(child: SingleChildScrollView(padding: const EdgeInsets.fromLTRB(18, 22, 18, 24),
    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Row(children: [IconButton(onPressed: () => Navigator.maybePop(context), icon: const Icon(Icons.arrow_back)), Expanded(child: Text(title, style: const TextStyle(fontSize: 26, fontWeight: FontWeight.w800)))]),
      Padding(padding: const EdgeInsets.only(left: 48), child: Text(subtitle, style: const TextStyle(color: Color(0xFF78909C), fontSize: 13))),
